...
for node in $SSH_IPS
do
  ssh -o "StrictHostKeyChecking no" -i ${SSH_KEY_PATH} ${SSH_USER}@$node "$MASTER_CONF_CMD ; $TSERVER_CONF_CMD ; $MASTER_CONF_RF_CMD ; $TSERVER_CONF_RF_CMD"
done
...
do
  ...
  if [[ "$MASTER_CRON_OK" == *${MASTER_CRON_PATTERN}* ]]; then
     ...
     ssh -o "StrictHostKeyChecking no" -i ${SSH_KEY_PATH} ${SSH_USER}@$node 'crontab -l | { cat; echo "*/3 * * * * /home/ec2-user/start_master.sh > /dev/null 2>&1"; } | crontab - '
...
if [ $num_zones -gt 1 ]; then
   ...
   ssh -o "StrictHostKeyChecking no" -i ${SSH_KEY_PATH} ${SSH_USER}@${SSH_IPS_array[0]} "$PLACEMENT_CMD"
...
for node in $SSH_IPS
do
  ssh -o "StrictHostKeyChecking no" -i ${SSH_KEY_PATH} ${SSH_USER}@$node "$TSERVER_YB_MASTER_ENV ; $TSERVER_START_CMD"
  TSERVER_CRON_OK="##";
  ...
  if [[ "$TSERVER_CRON_OK" == *${TSERVER_CRON_PATTERN}* ]]; then
     ...
     ssh -o "StrictHostKeyChecking no" -i ${SSH_KEY_PATH} ${SSH_USER}@$node 'crontab -l | { cat; echo "*/3 * * * * /home/ec2-user/start_tserver.sh > /dev/null 2>&1"; } | crontab - '
